...
def calculate_stock_bearish_patterns(
    stock_code: str,
    stock_name: str,
    table_name: str,
    ...
    """
    try:
        logger.info(f"开始计算股票空头组合: {stock_code} ({stock_name})")
        ...
        for dc in daily_chances:
            if dc.date:
                # 识别空头组合
                patterns = BearishPatternService.identify_bearish_patterns(
                ...
                if patterns:
                    # 多个组合用逗号连接
                    bearish_pattern = ','.join(patterns)
                    date_str = dc.date.strftime('%Y-%m-%d') if isinstance(dc.date, datetime) else str(dc.date)
                    updates.append((stock_code, date_str, bearish_pattern))
        ...
        # 批量更新
        updated_count = repository.update_bearish_pattern_batch(updates)
        logger.info(f"股票 {stock_code} 成功更新 {updated_count} 条记录的空头组合")
        
        return updated_count
        
    except Exception as e:
        logger.error(f"计算股票 {stock_code} 空头组合失败: {e}", exc_info=True)
        return 0
...
def main():
    ...
    try:
        ...
        for i, stock in enumerate(stocks, 1):
            ...
            try:
                updated = calculate_stock_bearish_patterns(
