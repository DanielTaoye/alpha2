...
def calculate_stock_volume_types(
    stock_code: str,
    stock_name: str,
    table_name: str,
    ...
    """
    try:
        logger.info(f"开始计算股票成交量类型: {stock_code} ({stock_name})")
        ...
        volume_type_map = VolumeTypeService.batch_calculate_volume_types(
        ...
        if not volume_type_map:
            logger.warning(f"股票 {stock_code} 未计算出任何成交量类型")
            return 0
        ...
        for dc in daily_chances:
            if dc.date:
                # 将date转换为datetime对象用于匹配
                dc_date = dc.date if isinstance(dc.date, datetime) else datetime.combine(dc.date, datetime.min.time())
                volume_type = volume_type_map.get(dc_date)
        ...
        # 批量更新
        updated_count = repository.update_volume_type_batch(updates)
        logger.info(f"股票 {stock_code} 成功更新 {updated_count} 条记录的成交量类型")
        
        return updated_count
        
    except Exception as e:
        logger.error(f"计算股票 {stock_code} 成交量类型失败: {e}", exc_info=True)
        return 0
...
def main():
    ...
    try:
        ...
        for i, stock in enumerate(stocks, 1):
            ...
            try:
                updated = calculate_stock_volume_types(
