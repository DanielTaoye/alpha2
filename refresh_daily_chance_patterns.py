"""刷新 b_daily_chance 表中的 volume_type, bullish_pattern, bearish_pattern"""
import sys
...
def get_daily_chance_records(conn, stock_code: str):
    """获取指定股票的所有 daily_chance 记录"""
    try:
        with conn.cursor(DictCursor) as cursor:
            query = """
...
def refresh_stock_patterns(conn, stock_code: str):
    """刷新指定股票的所有记录"""
    table_name = get_stock_table_name(stock_code)
    
    # 获取该股票的所有记录
    records = get_daily_chance_records(conn, stock_code)
    
    if not records:
        logger.info(f"股票 {stock_code} 没有记录，跳过")
        return 0
...
def main():
    ...
    try:
        ...
        # 询问是否继续
        user_input = input(f"\n是否开始刷新 {len(stock_codes)} 只股票的数据？(y/n): ")
        ...
        for idx, stock_code in enumerate(stock_codes, 1):
            ...
            updated = refresh_stock_patterns(conn, stock_code)
